...
An I2I objective should transfer more strongly to an I2T capability when the two objectives favor similar parameter updates.
...
(c) I2T gradient norms during I2I-only training. Blue denotes Jigsaw and purple Zoom-In. Norms are unnormalized and shown on a linear scale; checkpoints are equally spaced.
(d) Mean alignment and transfer across 15 sources for seven capabilities with more than 500 evaluation examples; the line is a least-squares fit.
(e) Alignment and transfer for all 105 source--target pairs, with illustrative pairs labeled. Both panels use uncentered alignment and accuracy gains over the I2T-only baseline; colors identify target families. The ellipse is a descriptive covariance contour, not a confidence region. Methods and sampling details are in Appendix~\ref{app:gradient_alignment}.}
...
We also track the magnitude of the I2T gradients as the model undergoes I2I-only training. As shown in Fig.~\ref{fig:grad_analysis}(c), the I2T gradient norms are lower at the end of I2I training, indicating that generation supervision also alters the gradient magnitude of I2T tasks.
\begin{findingbox}
In controlled I2I-I2T pairs, gradient alignment is strongest in early pre-attention normalization layers, and the I2T gradient magnitudes decrease overall during I2I training.
\end{findingbox}
...
We use the original 15 source checkpoints with gradient measurements and all seven I2T capabilities with more than 500 evaluation examples (Appendix~\ref{app:gradient-minibatch}).
For each capability, we sample 500 examples and compute gradients using minibatches of size 64.
For an I2I source $s$ and I2T target $t$, we compute the mean post-PCA gradient cosine ($G_{s,t}$) at the pretrained checkpoint using the pre-attention RMSNorm parameters concatenated across layers.
...
$$
Figure~\ref{fig:grad_analysis}(d) plots \((\bar{G}_t,\bar{\Delta}_t)\) for each of the seven capabilities.
We observe a strong positive correlation (\(r=0.953\)): capabilities whose gradients are more aligned with generation objectives on average also receive larger average gains from I2I training.
...
Figure~\ref{fig:grad_analysis}(e) plots each pair according to its gradient alignment ($G_{s,t}$) and downstream transfer gain ($\Delta_{s,t}$).
