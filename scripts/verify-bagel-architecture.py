...
for label in ('Q','K','V','Shared attention','Out','Pre-attn','Pre-MLP','RMSNorm','MLP','Gate','Up','Down','Q norm','K norm','ViT','Conn. in','Conn. out','2D pos.','VAE latent tokens','Text + ViT tokens'):
...
parameter_rects=[e for e in root.iter(ns+'rect') if e.get('class')=='parameter-box']
assert len(parameter_rects)==27,len(parameter_rects)
